...
namespace FrameItAPI.Services.services
{
    public class FolderService : IFolderService
    {
        ...
        public async Task<List<Folder>> GetSubFoldersByFolderId(int folderId)
        {
            return await _context.Folders
                .Where(f => f.ParentFolderId == folderId || folderId == 0 && f.ParentFolderId == null)
                .ToListAsync();
        }
        ...
        {
            return await _context.Folders.Where(f => f.Id == id && !f.IsDeleted).FirstOrDefaultAsync();;
        }
